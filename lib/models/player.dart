...
class PlayerModel extends ChangeNotifier {
  ...
  Future<ClosedCaptionFile>? _loadCaptions(VideoList item) {
    if (item.url.contains('youtu')) {
      return _loadYoutubeCaptionsFuture(item);
    }
  ...
  Future<ClosedCaptionFile>? _loadYoutubeCaptionsFuture(VideoList item) async {
    item.subs = item.url;
  ...
  Future<ClosedCaptionFile> getYoutubeSubtitles(String url) async {
    ...
    try {
      ...
      final en = manifest.getByLanguage("en").where((e) => !e.isAutoGenerated);
      if (en.isEmpty) {
        yt.close();
        return RawCaptionFile([]);
      }
